...
$ExpectedMsiSHA256  = ''
if ($env:usrExpectedMsiSHA256)
{ $ExpectedMsiSHA256 = $env:usrExpectedMsiSHA256.ToLower() 
}
$EnforceAuthSign    = ($env:usrAuthSign -eq 'true')

if ([string]::IsNullOrWhiteSpace($AutoConnect))
{ $AutoConnect = 'false' 
}
...
$installedApp = Get-ChildItem -Path $uninstKeys -ErrorAction SilentlyContinue | ForEach-Object {
    $props = Get-ItemProperty -Path $_.PsPath -Name DisplayName, DisplayVersion -ErrorAction SilentlyContinue
    if ($props.DisplayName -like 'OpenVPN Connect*')
    {
        $props
    }
...
if ($IsUpgrade)
{ Write-RMMLog "Detected existing OpenVPN Connect version: $installedVersion (upgrade scenario)" 
} else
{ Write-RMMLog "OpenVPN Connect not detected (fresh install)" 
}
...
{ Write-RMMLog "Cleanup warning: $($_.Exception.Message)" 'WARN' 
}
...
{ Write-RMMLog "Version check failed (non-fatal): $($_.Exception.Message)" 'WARN' 
}
...
try
{
    ...
    if ($listRCPre -eq 0)
    {
        ...
        {
            if ($line -match [Regex]::Escape($ProfileName))
            { $profileExists = $true; break 
            }
        }
    } else
    {
...
{ Write-RMMLog "profile-list pre-check failed: $($_.Exception.Message)" 'WARN' 
}
...
if (-not $profileExists)
{
    try
    {
        ...
        if ($importRC -ne 0)
        { throw "ovpnconnector import exit code $importRC" 
        }
    } catch
    {
...
try
{
    ...
    if (-not $found)
    { throw "Imported profile not found in profile-list" 
    }
...
if ($AutoConnect -eq 'true')
{
    try
    {
        ...
        if ($connRC -ne 0)
        { throw "profile-connect RC=$connRC" 
        }
    } catch
    {
